...
def aproximate_line(set_of_lines, degree, horizon1, horizon2, string):
    ...
    if len(set_of_lines) > 5:
        ...
        for x in ttt:
            if string == 'left':  # problerm gdy lekko w prawo
                if val1 >= int(polynomial(x)) >= val2:
                    poly_line.append([int(polynomial(x)), int(x)])  # powrot na (x,y)
            elif string == 'right':  # problem gdy lekko w lewo
...
def make_parser(test_path):
    ...
    parser.add_argument('--project', default=test_path, help='save results to project/name')
...
def detect(calibration_points):
    ...
    save_dir = str(opt.project + '/' + opt.name)  # increment run
    ...
    dataset.__iter__()
    path, img, im0s, vid_cap = dataset.__next__()  # (path, img, img_det, vid_cap)
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            h, w, _ = img_det.shape
            p = Path(p)  # to Path
            save_path = str((save_dir + '/' + p.name) if dataset.mode != 'stream' else str(opt.save_dir + '/' + 'web.mp4'))  # img.jpg
            ...
            for i in range(points_density):
                ...
                if i <= first_phase:
                    left_left_lane_points, left_lane_points, right_lane_points, right_right_lane_points = separate_points(points, left_left_lane_points, left_lane_points, right_lane_points, right_right_lane_points, img_middle[0])
                ...
                for point in points:
                    ...
                    if i >= second_phase:
                        right_lane_points, left_lane_points = finding_closest_point_by_width_and_height(point,
                                            last_points, howfar, right_lane_points, left_lane_points, i)
                last_points = points
            ...
            if approximation == 1:
                a1, b1, a2, b2 = 0, 0, 0, 0
                left_line = aproximate_line(set_of_lines_left, left_poly_degree, upper_horizon,
                                                    bottom_horizon, 'left')
                right_line = aproximate_line(set_of_lines_right, right_poly_degree, upper_horizon,
                    ...
                    for point in average_cars_points:
                        ...
                        if len(estimated_speed_list) > i:
                            ...
                            cv2.putText(img_det, (str(round(speed_towards_car, 1)) + "km/h"),
                                        (point[0] - 30, point[1] + 20),
                                        cv2.FONT_HERSHEY_COMPLEX_SMALL,
                                        font_size, [125, 246, 55], thickness=1)
                            if abs(real_dist[0]) < 0.75 and speed_towards_car > 2 * real_dist[
                                1]:  # real_dist[1] < 15 and speed_towards_car > 30:
...
if __name__ == '__main__':
    ...
    approximation = 1
    if calibrate == 1:
        calibration_points = camera_calibration(test_path + "/cal/calibration.png", test_path + "/cal/calibration.txt")
